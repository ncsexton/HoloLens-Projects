...
int receivingMessage(int initialSize, char* array, int socket, sockaddr_in recServer, socklen_t recServerLength)
{
	uint32_t totalSize;
	...
	while (delta > 0)
	{
		received = recvfrom(socket, (array + initialSize), 1460, 0, (sockaddr*)&recServer, &recServerLength);
		if (received > 0)
		{
...
/*int sendingMessage(int totalSize, int initialSize, char* array, int socket, sockaddr_in recServer, socklen_t recServerLength)
{
        uint32_t sent = 0;
        ...
        while (delta > 0)
        {
                sent = sendto(socket, (array + initialSize), delta, 0, (sockaddr*)&recServer, recServerLength);
                if (sent > 0)
                {
                        initialSize += sent;
                        delta = totalSize - initialSize;
...
int main()
{
	...
	socklen_t recServerLength = sizeof(recServer);
	...
	int result = bind(udpSocket, (sockaddr*)&hint, sizeof(hint));
	if (result < 0)
	{
	...
	do {//communicate until client closes connection
		result = recvfrom(udpSocket, receive, bufferLength, 0, (sockaddr*)&client, &clientSize);
		if (result > 1)
		{
			if (!(result = receivingMessage(result, receive, udpSocket, client, clientSize)));
			{
			...
			//Echo back
			sendResult = sendto(udpSocket, receive, result, 0, (sockaddr*)&hint, sizeof(hint));
